...
export default {
  ...
  methods: {
    highlightActiveTab() {
      ...
      const index = this.visitedViews.findIndex(item => item.fullPath === currentFullPath)

      if (index >= 0) {
        console.log(`Highlight check: Current route is ${currentFullPath}. Active tab set to ${this.editableTabsValue}.`)
      } else {
        console.log('Highlight check: No matching fullPath found for the current route.')
      }
    },
    // 刷新前缓存tab
    beforeUnload() {
    ...
    addTags() {
      const { name, path, fullPath, query } = this.$route
      if (!name) {
        return false
      }
      // 查找现有的以 `/admin/file` 开始的路径
      const existingIndex = this.visitedViews.findIndex(view => view.path.startsWith('/admin/file'))
      if (path.startsWith('/admin/file') && existingIndex !== -1) {
        // 更新现有标签的 path 和 fullPath
        this.visitedViews[existingIndex].path = path
        this.visitedViews[existingIndex].fullPath = fullPath
        this.visitedViews[existingIndex].query = query
      } else {
        // 添加新标签
